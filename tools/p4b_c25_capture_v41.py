#!/usr/bin/env python3
import concurrent.futures as cf, datetime as dt, hashlib, html, json, pathlib, re, time, urllib.parse, urllib.request, zipfile
UA='Mozilla/5.0 (compatible; P4B-C25/4.1)'
LIST='https://www.fsc.gov.tw/ch/home.jsp?id=96&parentpath=0%2C2&mcustomize='
BASE='https://www.fsc.gov.tw/ch/'
FROZEN=14638
ID_SHA='385428f7486b385f61a5c1e0aefaa5f459b87cb69c0cc287efa8bbe41920da78'
ROW_SHA='bcaca26cc93d7ace99bf70dc2d94adb03737f589e24b4c2ce452dc4c0778f8d2'
...
def main():
    ...
    delta=tr-FROZEN; lo,hi=1+delta,225+delta; plo=(lo-1)//15+1; phi=(hi-1)//15+1
    pages={}
    ...
    if [r['record_no'] for r in rows] != list(range(1,226)): raise SystemExit('record set mismatch')
    ...
    m={'schema':'p4b_fsc_recovery_chunk_manifest_v41','chunk':'C25','expected_record_count':225,'captured_record_count':len(manifest),'failure_count':0,'identity_sha256':ids,'fullrow_sha256':full,'raw_bytes':sum(x['raw_bytes'] for x in manifest),'records':manifest,'status':'CAPTURED_VALIDATED_PENDING_DRIVE_READBACK','generated_at':nowz()}
    (root/'P4B_FSC_RECOVERY_C25_MANIFEST.json').write_text(json.dumps(m,ensure_ascii=False,indent=2,sort_keys=True)+'\n')
    pkg=pathlib.Path('out/P4B_FSC_RECOVERY_C25_V41.zip')
    with zipfile.ZipFile(pkg,'w',zipfile.ZIP_DEFLATED,compresslevel=6) as z:
        for p in sorted(root.rglob('*')):
            if p.is_file(): z.write(p,p.relative_to(root))
    receipt={'chunk':'C25','package':pkg.name,'bytes':pkg.stat().st_size,'sha256':hashlib.sha256(pkg.read_bytes()).hexdigest(),'record_count':225,'identity_sha256':ids,'fullrow_sha256':full,'status':'CAPTURED_VALIDATED_PENDING_DRIVE_READBACK'}
    pathlib.Path('out/P4B_FSC_RECOVERY_C25_V41_RECEIPT.json').write_text(json.dumps(receipt,indent=2,sort_keys=True)+'\n')
