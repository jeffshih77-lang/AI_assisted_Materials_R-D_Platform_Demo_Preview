...
def load_checkpoint_keys(path):
    ...
    out=set()
    for x in arr:
        if isinstance(x,dict): out.add((str(x['code']),int(x['qryType'])))
        else:
            c,q=x; out.add((str(c),int(q)))
    if len(out)!=621: raise RuntimeError(f'checkpoint_key_count_not_621 {len(out)}')
...
if mode=='t05':
    ...
    for run_id in repair_runs: ns['wait_run'](run_id)
    ...
    if len(target)!=4872 or not checkpoint <= target: raise RuntimeError('t05_target_or_checkpoint_invalid')
    remaining_target=target-checkpoint
    if len(remaining_target)!=4251: raise RuntimeError(f'remaining_target_not_4251 {len(remaining_target)}')
    store={}; audit=[]
    for run_id in repair_runs:
        arts=ns['list_artifacts'](run_id); print(json.dumps({'run':run_id,'artifacts':len(arts)}),flush=True)
        for a in arts:
            blob=ns['download_artifact'](a); res=ns['parse_formal_artifact'](a,blob,'t05',store)
    ...
    if overlap: raise RuntimeError(f'repair_checkpoint_overlap {len(overlap)} sample={sorted(overlap)[:5]}')
    unexpected=set(store)-remaining_target
    if unexpected: raise RuntimeError(f'repair_outside_remaining_target {len(unexpected)} sample={sorted(unexpected)[:5]}')
    ...
    for i,key in enumerate(missing,1):
        body,raw,gz,meta=ns['capture_t05'](key); ns['add_item'](store,key,body,raw,gz,meta); print(json.dumps({'repair':i,'of':len(missing),'key':key}),flush=True)
    if set(store)!=remaining_target: raise RuntimeError(f'final_remaining_coverage_fail have={len(store)} target={len(remaining_target)}')
    if checkpoint & set(store) or len(checkpoint|set(store))!=4872: raise RuntimeError('final_union_coverage_fail')
    summary={'schema':'w08_t05st09_final_reference_aggregate_v2','work_unit_id':'W08-RAW-MOPS-DIVIDEND-T05ST09','formal_raw':True,'status':'FORMAL_RAW_COMPLETE','endpoint':ns['BASE']+'/mops/web/ajax_t05st09_2','universe_count':2436,'universe_sha256':universe_sha,'target_units':4872,'checkpoint_units':621,'remaining_units':4251,'remaining_success_units':len(store),'union_success_units':4872,'missing_repaired_units':len(missing),'checkpoint_key_ledger_sha256':checkpoint_key_sha,'checkpoint_reference_mode':'IMMUTABLE_DRIVE_SHARDS_NO_REGRAB','checkpoint_drive_shards':T05_CHECKPOINT_DRIVE,'source_run_ids':list(repair_runs),'contract':{'date1':'085','date2':'115','qryType':[1,2],'firstin':'1','TYPEK':'all'},'artifact_audit':audit,'generated_at':ns['now']()}
    root=pathlib.Path('out_w08_final_t05'); root.mkdir(parents=True,exist_ok=True)
    out=root/'RAW-W08_MOPS_DIVIDEND_T05ST09_FORMAL_RAW_REMAINING_4251_FINAL_20260912.zip'; ns['deterministic_zip'](out,store,'t05',summary)
    ...
    for run_id in run_ids: ns['wait_run'](run_id)
    ...
    for run_id in run_ids:
        arts=ns['list_artifacts'](run_id); print(json.dumps({'run':run_id,'artifacts':len(arts)}),flush=True)
        for a in arts:
            blob=ns['download_artifact'](a); res=ns['parse_formal_artifact'](a,blob,'pre',store)
            res.update({'run_id':run_id,'artifact_id':a['id'],'digest':a.get('digest')}); audit.append(res); print(json.dumps(res),flush=True)
    ...
    out=root/'RAW-W08_MOPS_EXRIGHT_PRE20050505_FORMAL_RAW_ROC85_94_FINAL_20260912.zip'; ns['deterministic_zip'](out,store,'pre',summary)
