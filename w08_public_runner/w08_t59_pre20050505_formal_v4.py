...
SHARD=int(os.environ.get('SHARD','0')); SHARDS=int(os.environ.get('SHARDS','64'))
ROOT=pathlib.Path(f'out_t59_pre_v4_{SHARD:02d}_of_{SHARDS:02d}'); (ROOT/'raw').mkdir(parents=True,exist_ok=True); (ROOT/'request').mkdir(exist_ok=True)
BASE='https://mopsov.twse.com.tw'; FRONT='/mops/web/t59sb07'; AJAX='/mops/web/ajax_t59sb07'; UA='Mozilla/5.0 (compatible; W08T59PreFormal/4.0)'; YEARS=[f'{y:03d}' for y in range(85,95)]; SESSION_BATCH=30
def now(): return dt.datetime.now(dt.timezone.utc).isoformat().replace('+00:00','Z')
def sha(b): return hashlib.sha256(b).hexdigest()
def load_codes():
 s=pathlib.Path('w08_public_runner/w08_pre20050504_codes_1281.txt.gz.b64').read_text().strip(); raw=gzip.decompress(base64.b64decode(s)); codes=raw.decode().splitlines(); assert len(codes)==1281 and len(set(codes))==1281 and all(len(c)==4 and c.isdigit() for c in codes); return codes,sha(raw)
def open_session():
 last=None
 for a in range(1,9):
  try:
   ...
   return (op,fu,[c.name for c in cj],sha(fb),len(fb))
...
def body_for(code,year):
 return urllib.parse.urlencode([('encodeURIComponent','1'),('step','1'),('firstin','1'),('off','1'),('keyword4',''),('code1',''),('TYPEK2',''),('checkbtn',''),('queryName','co_id'),('inpuType','co_id'),('TYPEK','all'),('co_id',code),('year',year),('month',''),('b_date',''),('e_date','')]).encode('ascii')
...
def capture(code,year,session):
 body=body_for(code,year); stem=f'{code}_{year}'; (ROOT/'request'/f'{stem}.body').write_bytes(body); retries=[]; last=None
 for a in range(1,9):
  try:
   if session is None: session=open_session()
   op,fu,cookies,front_sha,front_bytes=session
   ...
   last=repr(e); retries.append({'attempt':a,'at':now(),'error':last}); session=None
   if a>=8: return None,{'code':code,'year':year,'request_body_sha256':sha(body),'retries':retries,'terminal_error':last},None
 ...
 return {'code':code,'year':year,'retrieved_at':ts,'request_url':BASE+AJAX,'method':'POST','request_body_bytes':len(body),'request_body_sha256':sha(body),'http_status':st,'final_url':final,'response_bytes':len(rb),'response_sha256':sha(rb),'gzip_sha256':sha(gz),'gzip_roundtrip':True,'classification':cls,'retry_count':len(retries),'retries':retries,'headers':hdr,'session_front_sha256':front_sha,'session_front_bytes':front_bytes},None,session
codes,universe_sha=load_codes(); full=[(c,y) for c in codes for y in YEARS]; assert len(full)==12810
assigned=[x for i,x in enumerate(full) if i%SHARDS==SHARD]; records=[]; failures=[]; session=None
for i,(c,y) in enumerate(assigned,1):
 if (i-1)%SESSION_BATCH==0: session=None
 rec,fail,session=capture(c,y,session)
 ...
 time.sleep(.04+random.random()*.06)
manifest={'schema':'w08_t59_pre20050505_formal_v4_0','work_unit_id':'W08-RAW-MOPS-EXRIGHT-PRE20050505','formal_raw':True,'endpoint':BASE+AJAX,'front':BASE+FRONT,'official_scope':'公開發行及94.5.5前之全體公司','transport':{'session_reuse_max_units':SESSION_BATCH,'refresh_on_error':True},'contract':{'years':YEARS,'firstin':'1','TYPEK':'all','company_required':True,'year_required':True,'ui_click_semantics_verified_each_session':True,'ui_sequence':'doAction sets firstin=1 before ajax1(form1)'},'universe_count':len(codes),'universe_sha256':universe_sha,'target_units':len(full),'shard':SHARD,'shards':SHARDS,'assigned_units':len(assigned),'success_units':len(records),'failed_units':len(failures),'records':records,'failures':failures}
(ROOT/'MANIFEST.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2,sort_keys=True)+'\n')
(ROOT/'COVERAGE.json').write_text(json.dumps({k:manifest[k] for k in ['work_unit_id','official_scope','universe_count','universe_sha256','target_units','shard','shards','assigned_units','success_units','failed_units']},ensure_ascii=False,indent=2,sort_keys=True)+'\n')
...
print(json.dumps({'shard':SHARD,'assigned':len(assigned),'success':len(records),'failed':len(failures),'universe_sha256':universe_sha},ensure_ascii=False),flush=True)
