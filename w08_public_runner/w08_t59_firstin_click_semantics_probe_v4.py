import datetime as dt,gzip,hashlib,http.cookiejar,json,pathlib,re,time,urllib.parse,urllib.request,random
OUT=pathlib.Path('w08_t59_firstin_click_probe_v4_out'); OUT.mkdir(exist_ok=True)
BASE='https://mopsov.twse.com.tw'; FRONT='/mops/web/t59sb07'; AJAX='/mops/web/ajax_t59sb07'; UA='Mozilla/5.0 (compatible; W08T59ClickSemanticsProbe/4.0)'
...
def session():
 ...
 for a in range(1,9):
  try:
   ...
   if st!=200 or not fb: raise RuntimeError(f'front bad status={st} bytes={len(fb)}')
   return op,fu,fb
  except Exception as e:
   last=repr(e); time.sleep(min(20,1.6**a+random.random()*2))
 raise RuntimeError('session_exhausted '+str(last))
...
def capture(label,code,year,firstin):
 ...
 for a in range(1,9):
  try:
   ...
   if st!=200 or not rb: raise RuntimeError(f'post bad status={st} bytes={len(rb)}')
   break
  except Exception as e:
   retries.append({'attempt':a,'at':now(),'error':repr(e)})
   if a==8: raise
   time.sleep(min(20,1.6**a+random.random()*2))
...
man={'schema':'w08_t59_firstin_click_semantics_probe_v4','qualification_only':True,'front_sha256':sha(front),'front_bytes':len(front),'ui_evidence':{'hidden_initial':'ture','click_doAction_sets_firstin':'1','query_button_calls':'doAction();ajax1(document.form1,table01)'},'records':recs,'result':'PASS' if recs[2]['table_count']>0 and not recs[2]['security_shell'] else 'CONTROL_FAIL'}
