...
# These jobs were already observed in_progress before this dispatcher was committed.
ALREADY_RUNNING={30,31,34,37,39,41,44}
# These jobs were observed queued. Shard 63 becomes the one-runner leader for all of them.
...
if os.environ.get('TAIL_CHILD')=='1':
    ...
    (root/'DEFERRED_TO_SHARD63_BULK.json').write_text(json.dumps({'formal_raw':False,'status':'DEFERRED_TO_SHARD63_BULK','leader_shard':LEADER,'reason':'This matrix job was still queued when single-runner bulk acceleration was frozen; this job issues zero MOPS requests.'},sort_keys=True)+'\n')
    print('DEFERRED_TO_SHARD63_BULK_NO_REQUESTS',flush=True)
    raise SystemExit(1)
elif SHARD==LEADER:
    ensure_core()
    results=[]
    ...
    # Individual shard failures are retained as formal partial artifacts and repaired by final reconciler.
    # Leader exits nonzero only after every child has finished, so artifact upload still captures all evidence.
    if any(rc!=0 for _,rc in results): raise SystemExit('BULK_HAS_PARTIAL_FAILURES')
else:
    # Only applies to ALREADY_RUNNING shards if a later rerun occurs; preserves frozen one-shard semantics.
    run_core_in_process()
